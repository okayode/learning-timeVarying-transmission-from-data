...
class PINN_ExpBeta:
    ...
    def __init__(self, t, I, R, layers1, layers2, M, b0, xi, lb, ub):
        ...
        self.beta_pred = self.betaFunc(self.t_tf)
        ...
        self.loss = tf.reduce_mean(tf.square(self.I_tf - self.I_pred)) + \
            ...
            tf.reduce_mean(tf.square(1.0 - self.b_pred[0])) + \
            tf.reduce_mean(tf.square(self.b0 - self.beta_pred[0:self.M])) + \
    ...
    def b_net(self,t):
        M = self.M
        b = self.neural_net2(t, self.layers2, self.weights2, self.biases2)
        eta = self.eta
        b = tf.math.exp(-eta*b)
    ...
    def train(self, nIter):
        ...
        for it in tqdm(range(nIter)):
            ...
            if it % 100 == 0:
               ...
               eta_value = self.sess.run(self.eta)
...
model = PINN_ExpBeta(t_train, I_train, R_train, layers1, layers2, 528, 0.279, 0.49, lb, ub)
model.train(niter)
...
eta_value = model.sess.run(model.eta)
...
# learned parameters
print("eta:",*["%.8f"%(x) for x in eta_value])
